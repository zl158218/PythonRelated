...
def dates(company):
    ...
    for page in range(0, 20, 10):
        try:
            url = 'https://www.baidu.com/s?ie=utf-8&cl=2&rtt=1&bsst=1&tn=news&word=' + company + \
                '&x_bfe_rqs=03E80&x_bfe_tjscore=0.012060&tngroupname=organic_news&pn=' + \
                str(page)
            ...
            if res.status_code == 200:
                ...
                for i in range(len(info)):
                    if len(info[i]) > 200:
                        ...
                        source = re.findall(
                            '<img.*?/>(.*?)$', info[i], re.S)[0].split('&nbsp;&nbsp;')[0].strip()
                        time = re.findall(
                            '<img.*?/>(.*?)$', info[i], re.S)[0].split('&nbsp;&nbsp;')[1].strip()
                        sources.append(source)
                        Release_time.append(time)
                    else:
                        sources.append(info[i].split(
                            '&nbsp;&nbsp;')[0].strip())
                        Release_time.append(info[i].split(
                            '&nbsp;&nbsp;')[1].strip())
                ...
                # 写入txt文件
                file1 = open(
                    r'/media/aim/a61c08f4-b2ff-497d-bdf6-9efe8867b0a1/all_tmp/百度搜索结果链接爬虫/test.txt', 'a')
                file1.write(company + '消息监控completed！' + '\n' + '\n')
                for i in range(len(titles)):
                    file1.write(str(
                        i) + ". " + titles[i] + "  " + "(" + sources[i] + "  " + Release_time[i] + ")" + '\n')
