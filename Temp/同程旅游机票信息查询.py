...
def citys_list(headers):
    ...
    if city_json.status_code == 200:
        city_data = json.loads(re.findall('tc1818278580\((.*?)\)', str(city_json.text))[0])['citys']
...
def datas(data):
    ...
    for f1 in range(len(flight)-1):
        airlines = flight[f1].split(',')[1] + "--" +flight[f1].split(',')[2]  # 航空公司
        ticket_data = flight[f1].split(',')[19]                               # 机票日期

        # 起飞及降落时间
        take_off_airplane = flight[f1].split(',')[3] +" ---> " + flight[f1].split(',')[5]                            
    ...
    for a0 in range(len(all_price)):
        try:
            ...
            print()
            preferential = all_price[a0].split(",")[22][-14:-6].replace('^',"") + "--￥" + all_price[a0].split(",")[0]   # 同程特惠
            economy_class = all_price[a0].split(",")[66][-14:-6].replace('^',"") + "--￥" + all_price[a0].split(",")[55]  # 经济舱
            premium_economy = all_price[a0].split(",")[110][-14:-6].replace('^',"") + "--￥" + all_price[a0].split(",")[110][-4:]   # 高端经济舱                                                                                                     # 
            full_fare_economy = all_price[a0].split(",")[132][-14:-6].replace('^',"") + "--￥" + all_price[a0].split(",")[99] # 全价经济舱                                                                                                     # 
            return_package = all_price[a0].split(",")[154][-14:-6].replace('^',"") + "--￥" + str(int(all_price[a0].split(",")[132][-4:].replace("*","")) - int(all_price[a0].split(",")[147]))  # 往返优惠
...
if __name__ == '__main__':
    ...
    # print(city['straing'],city['destination'],city['date'])
    # # city = citys_list(headers)
    # # straing_code = city['straing']
    # # end_code = city['destination']
    # # date = city['date']
    # # print(req_data)
    # # datas(req_data)
...
#         while True:
#             city_url = "https://www.ly.com/Flight/FlightBookAjax.aspx?Type=GetAllCity&_dAjax=callback&callback=tc1818278580"
#             datas = json.loads(re.findall('tc1818278580\((.*?)\)', str(requests.get(city_url, headers=header).text))[0])
#             straing_point = str(input("请输入起飞城市（出发地）： "))
#             destination = str(input("请输入降落城市（目的地）： "))
#             date_s = str(input("请输入出发时间（例如：2019-03-10）： "))
#             if datas.get('citys').get(straing_point).get('code'):
#                 straing_codes = datas.get('citys').get(straing_point).get('code')
#                 destination_code = datas.get('citys').get(destination).get('code')
#                 citys_dict['straing'] = straing_codes
#                 citys_dict['destination'] = destination_code
#                 citys_dict['date'] = date_s
#                 break
#     except:
#         print("输入有误，请重新输入")
...
#     end_code = city['destination']
#     date = city['date']
#     print(straing_code,end_code,date)
...
#     main()
    ...
    # req_data = ticket(straing_code,end_code,date)
    # print(req_data)
    # datas(req_data)
...
# datas = re.findall('tc1818278580\((.*?)\)', str(requests.get(city_url, headers=header).text))[0]
# req = requests.get(url,headers=headers).text
# print("找不到该城市,请重新输入...")    except:    try:
# print("找不到该城市,请重新输入...")
